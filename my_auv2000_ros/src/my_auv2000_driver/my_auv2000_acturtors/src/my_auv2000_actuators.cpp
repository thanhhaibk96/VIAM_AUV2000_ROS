...
//CANBUS
static int socketCAN;
static struct sockaddr_can SocketCAN_addr;
static struct ifreq CAN_ifr;
...
uint8_t UCAN_Send(const int& _socket, const uint32_t& _id, const uint8_t& _buffer_size, const uint8_t* _data);
void UCAN_ReadData(void);
...
void onCANBUSHandleFrames(const can_frame &f)
{  
//  ROS_INFO("onCANBUSHandleFrames");
  ...
  // REQ ACK
  if(((f.can_id == AUV_MASS_SHIFTER) || (f.can_id == AUV_ARMBOARD_1) || (f.can_id == AUV_ARMBOARD_2) || (f.can_id == AUV_PISTOL) || (f.can_id == AUV_THRUSTER))
     && (f.data[7] == UCAN_Checksum(CANBUS_Data_NoChecksum, 0, 7))
     && (f.data[0] == 'R') && (f.data[1] == 'E') && (f.data[2] == 'Q') && (f.data[3] == 'A') && (f.data[4] == 'C') && (f.data[5] == 'K'))
  {
     ROS_INFO_STREAM("CAN ID " << std::to_string(f.can_id) << ": Received ACK Successfully");
      CANBUS_ACK = true;
  }

  if((f.can_id == AUV_MASS_SHIFTER) && (f.data[7] == UCAN_Checksum(CANBUS_Data_NoChecksum, 0, 7)))
  {
  ...
  if((f.can_id == AUV_PISTOL) && (f.data[7] == UCAN_Checksum(CANBUS_Data_NoChecksum, 0, 7)))
  {
    if((CANBUS_Data_NoChecksum[0] == 'R') && (CANBUS_Data_NoChecksum[1] == 'P') && (CANBUS_Data_NoChecksum[2] == 'V'))
    {
  ...
  if((f.can_id == AUV_THRUSTER) && (f.data[7] == UCAN_Checksum(CANBUS_Data_NoChecksum, 0, 7)))
  {
    if((CANBUS_Data_NoChecksum[0] == 'R') && (CANBUS_Data_NoChecksum[1] == 'P') && (CANBUS_Data_NoChecksum[2] == 'V'))
    {
  ...
  if((f.can_id == AUV_ARMBOARD_1) &&(f.data[7] == UCAN_Checksum(CANBUS_Data_NoChecksum, 0, 7)))
  {
    switch(f.data[1])
    {
      case STATUS_DATA:
  ...
  if((f.can_id == AUV_ARMBOARD_2) &&(f.data[7] == UCAN_Checksum(CANBUS_Data_NoChecksum, 0, 7)))
  {
    switch(f.data[1])
    {
      case STATUS_DATA:
...
void UCAN_RequestData(AUV_BOARD_TypeDef _type_board)
{
  switch (_type_board) {
    case AUV_ARMBOARD_1:
    {
      ...
      UCAN_Send(socketCAN, AUV_ARMBOARD_1, 8, ROS_CANBUS_Data);
      break;
    }
    case AUV_ARMBOARD_2:
    {
      ...
      ROS_CANBUS_Data[6] = 0x0A;
      ROS_CANBUS_Data[7] = UCAN_Checksum(ROS_CANBUS_Data, 0, 7);
      UCAN_Send(socketCAN, AUV_ARMBOARD_2, 8, ROS_CANBUS_Data);
      break;
    }
    case AUV_THRUSTER:
    {
      ...
      ROS_CANBUS_Data[6] = 0x0A;
      ROS_CANBUS_Data[7] = UCAN_Checksum(ROS_CANBUS_Data, 0, 7);
      UCAN_Send(socketCAN, AUV_THRUSTER, 8, ROS_CANBUS_Data);
      break;
    }
    case AUV_MASS_SHIFTER:
    {
      ...
      ROS_CANBUS_Data[6] = 0x0A;
      ROS_CANBUS_Data[7] = UCAN_Checksum(ROS_CANBUS_Data, 0, 7);
      UCAN_Send(socketCAN, AUV_MASS_SHIFTER, 8, ROS_CANBUS_Data);
      break;
    }
    case AUV_PISTOL:
    {
      ...
      ROS_CANBUS_Data[6] = 0x0A;
      ROS_CANBUS_Data[7] = UCAN_Checksum(ROS_CANBUS_Data, 0, 7);
      UCAN_Send(socketCAN, AUV_PISTOL, 8, ROS_CANBUS_Data);
      break;
    }
  }
}
...
uint32_t _count_restart_fd = 0;
uint8_t UCAN_Send(const int& _socket, const uint32_t& _id, const uint8_t& _buffer_size, const uint8_t* _data)
{
  mutexSocketCAN.lock();
  _count_restart_fd++;

  if(_count_restart_fd >= 200)
  {
  ...
  struct can_frame _can_frame;
  memset(&_can_frame, 0x0, sizeof(_can_frame));

  _can_frame.can_id = _id;
  _can_frame.can_dlc = _buffer_size;

  for(uint8_t i = 0; i < _buffer_size; i++)
  {
    _can_frame.data[i] = _data[i];
  }
...
//     return 0;
//   }
  if (send(_socket, &_can_frame, sizeof(struct can_frame), 0))
  {
     perror("Write");
     mutexSocketCAN.unlock();
     ros::Duration(0.01).sleep();
...
void UCAN_ReadData(void)
{
  ...
  do
  {
    nbytes = read(socketCAN, &_can_frame, sizeof(struct can_frame));
   _timeout--;
   // ROS_INFO_STREAM("Timeout: " << _timeout);
    if (nbytes < 0) {  
      //  _timeout = 0;    
       continue;
    }

   onCANBUSHandleFrames(_can_frame);
   // ROS_INFO_STREAM("Data: " << _can_frame.can_id << " " << _can_frame.data[0] << " " << _can_frame.data[1] << " " << _can_frame.data[2]
   //                 << " " << _can_frame.data[3] << " " << _can_frame.data[4] << " " << _can_frame.data[5] << " " << _can_frame.data[6] << " " << _can_frame.data[7]);
...
void UCAN_InitSocket(void)
{
   memset(&CAN_ifr, 0x0, sizeof(CAN_ifr));

   if ((socketCAN = socket(PF_CAN, SOCK_RAW, CAN_RAW)) < 0) {
       perror("Socket Error!!!");
      //  return 1;
   }
   // strcpy(CAN_ifr.ifr_name, CAN_Device.c_str());
   strcpy(CAN_ifr.ifr_name, "can0");
   ioctl(socketCAN, SIOCGIFINDEX, &CAN_ifr);

   memset(&SocketCAN_addr, 0, sizeof(SocketCAN_addr));
   SocketCAN_addr.can_family = AF_CAN;
   SocketCAN_addr.can_ifindex = CAN_ifr.ifr_ifindex;

   if (bind(socketCAN, (struct sockaddr *)&SocketCAN_addr, sizeof(SocketCAN_addr)) < 0) {
     perror("Bind SocketCAN");
      //  return 1;
   }

   struct can_filter rfilter[1];
   rfilter[0].can_id   = 0x120;
   rfilter[0].can_mask = 0x128;
   // Received Datas That have ID 0x121 - 0x127
